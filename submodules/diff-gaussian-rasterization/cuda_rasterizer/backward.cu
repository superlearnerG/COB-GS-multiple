...
renderCUDA(
	...
	const float* __restrict__ dL_dpixels_mask,
	const float* __restrict__ dL_invdepths,
	float3* __restrict__ dL_dmean2D,
	float2* __restrict__ dL_dmask_signal,
...

{
	...
	if (inside)
	{
		...
		if(include_mask)
		dL_dpixel_mask = dL_dpixels_mask[pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (include_mask){
			    atomicAdd(&(dL_dmask[global_id]), dchannel_dcolor * dL_dpixel_mask);

                if (dL_dpixel_mask < 0) {
                    atomicAdd(&dL_dmask_signal[global_id].x, 1.0f);
                } else if (dL_dpixel_mask > 0) {
                    atomicAdd(&dL_dmask_signal[global_id].y, 1.0f);
                }
			}
