	...
	const bool include_mask)
{
  ...
  if (include_mask) {
	out_mask = torch::full({1, H, W}, 0.0, float_opts);
  }
  else {
	out_mask = torch::full({1}, 0.0, float_opts);
  }
  ...
  return std::make_tuple(rendered, out_color, out_mask, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth);
}
	...
	const bool include_mask)
{
  ...
  torch::Tensor dL_dmask;
  torch::Tensor dL_dmask_signals;
  if (include_mask) {
	dL_dmask = torch::zeros({P}, means3D.options());
	dL_dmask_signals = torch::zeros({P, 2}, means3D.options());
  } else {
	dL_dmask = torch::zeros({1}, means3D.options());
	dL_dmask_signals = torch::zeros({1}, means3D.options());
  }
  ...
  {  
	  ...
	  dL_dmask_signals.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dmask_signals, dL_dcolors, dL_dmask, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
